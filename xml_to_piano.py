...
dst_dir = '/deepiano_data/zhaoliang/xml_wav/xml_arachno'
...
random.seed(6)
xml_list = random.sample(all_bgm_xml_list, 400)
print('采样数量', len(xml_list))
# print(xml_list[11])
error_file = []
if __name__ == '__main__':
    cnt = 0
    for file in xml_list:
        ...
        m = converter.parse(os.path.join(dir_, file))
        dst_midi_fn = dst_dir+'/'+ '%03d.mid'%cnt
        try:
            m.write('midi', dst_midi_fn)
            print('midi_fn: ', dst_midi_fn)
